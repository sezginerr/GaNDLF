...
class pix2pixHD(ModelBase):
    ...
    def forward(self):
        input_label, _ , _ , feat_map = self.encode_input(self.real_B,real_image=self.real_A)
    ...
    def preprocess(self,img,label):
        label=label.float()
        label=(label-self.output_range[0])/(self.output_range[1]-self.output_range[0])
        img=(img-self.input_range[0])/(self.input_range[1]-self.input_range[0])
